package com.razzaghi.noteapp.presentation.ui.add
...
import com.razzaghi.noteapp.presentation.ui.add.view_model.AddAction
import com.razzaghi.noteapp.presentation.ui.add.view_model.AddEvent
import com.razzaghi.noteapp.presentation.ui.add.view_model.AddState
...
fun AddNoteScreen(
    ...
    action: Flow<AddAction>,
    state: AddState,
    events: (AddEvent) -> Unit,
    ...
    if (state.notificationBottomSheet == UIComponentState.Show) {
        NotificationBottomSheet(onDismiss = { events(AddEvent.OnUpdateNotificationBottomSheet(UIComponentState.Hide)) })
    }


    if (state.reminderDialog == UIComponentState.Show) {
        ReminderDialog(onDismiss = { events(AddEvent.OnUpdateReminderDialog(UIComponentState.Hide)) }) { calendar ->
            events(AddEvent.AddReminder(calendar))
        }
    }
    ...
    DefaultScreenUI(
        ...
        topBar = {
            AddNoteToolbar({ popUp(false) }, {
                events(AddEvent.OnUpdateReminderDialog(UIComponentState.Show))
            })
        },
        bottomBar = {
            AddNoteBottomBar(enabled = state.title.isNotEmpty(), onAddExecute = {
                if (state.reminder == null) {
                    events(AddEvent.AddNote(null))
                } else {
                    if (hasPermission) {
                        val id = insertEvent(
                            context,
                        ...
                        events(AddEvent.AddNote(eventId = id))
                    } else {
                        launcher.launch(Manifest.permission.WRITE_CALENDAR)
                    }
                }
            }, onLabel = {
                events(AddEvent.OnUpdateLabel("Work"))
            })
        },
        onTryAgain = { events(AddEvent.OnRetryNetwork) },
    ) {
        ...
        Column(
            ...
            TextField(
                ...
                value = state.title,
                onValueChange = {
                    events(AddEvent.OnUpdateTitle(it))
                },
            ...
            TextField(
                ...
                value = state.note,
                onValueChange = {
                    events(AddEvent.OnUpdateNote(it))
                },
                keyboardOptions = KeyboardOptions(
...
@Composable
fun AddNoteBottomBar(enabled: Boolean, onAddExecute: () -> Unit, onLabel: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
...
@Composable
fun AddNoteToolbar(onBack: () -> Unit, onNotif: () -> Unit) {
...
private fun HomeScreenHome() {
    NoteAppTheme {
        AddNoteScreen(emptyFlow(), emptyFlow(), AddState(), {}, {})
    }
}